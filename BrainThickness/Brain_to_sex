...
def logireg (X1,Y1,X2,Y2, lr,epoch, dr, batch_size):
    cols=X1.shape[1]
    train_n=X1.shape[0]
    ...
    with tf.Session() as sess:
        ...
        for step in range(epoch):
            for j in range(total_batch):
                ...
                cost_val, _ = sess.run([cost,train], feed_dict={x: batch_x, y: batch_y, dropout_rate:dr})
            # Showing loss values
            if (step+1)%100==0:
                print(step+1,cost_val)
            # Showing interim accuracy
            if (step+1)%500==0:
                mid_h, mid_c, mid_a = sess.run([hypothesis, predicted, accuracy], feed_dict={x: X2, y: Y2, dropout_rate: 1.0})
                print("\n Hypothesis: ", mid_h, "\nCorrect (Y) ", mid_c, "\nAccuracy: ", mid_a)
            ...
            if np.isnan(cost_val):
                ...
                break
        saver.save(sess,path+"brain-sex")
        # accuracy report
...
logireg(train_x, train_y, test_x, test_y,lr=0.000001,epoch=20000,dr=0.7, batch_size=200)
